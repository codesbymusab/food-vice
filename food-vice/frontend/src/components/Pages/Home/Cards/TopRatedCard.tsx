...
export function TopRatedCard({ restaurant }: { restaurant: TopRatedRestaurant }) {
    ...
    return (
        <div className="flex gap-4 p-4 bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-800 hover:shadow-xl transition-shadow cursor-pointer" onClick={() => navigate(`/explore/restaurant/${restaurant._id}`)}>
            ...
            <div className="flex-1">
                ...
                <p className="text-xs text-slate-500 mt-2">{restaurant.cuisines.slice(0, 3).map((cuisine) => {
                    return `• ${cuisine} `
                })}</p>
              
              
                <div className="flex items-center gap-1 mt-2 text-xs font-semibold text-primary">
                     <span className="text-accent-cyan">{restaurant.isOpen ? 'Open Now' : `Closed • ${restaurant.openingTime && `Opens ${restaurant.openingTime}`} `}</span>{restaurant.priceCategory!=='' ? `  •${restaurant.priceCategory}`:''}
                </div>
            </div>
        </div>
    )
